// src/controllers/user/changeRequest.controller.ts
...
export const changeRequestController = {
  ...
  async updateRequestStatus(req: Request, res: Response) {
    try {
      ...
      const { request, updatedUser } = await changeRequestService.updateStatus(requestId, status, message);

      res.status(200).json({
        success: true,
        message: `Request ${status}`,
        data: { request, partner: updatedUser }
      });
    } catch (err: any) {
      res.status(500).json({ success: false, message: err.message });
    }
