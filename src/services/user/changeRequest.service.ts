// src/services/user/changeRequest.service.ts
...
export const changeRequestService = {
  ...
  async getPendingRequestsForAdmin(partnerId: string) {
    return ChangeRequestModel.find({ userId: partnerId, status: "pending" })
  ...
  async updateStatus(
    ...
    if (status === "approved") {
      ...
      if (request.requestType === "bankDetails") {
        ...
        // pull raw object and merge
        const existingDocs = user.documents?.toObject
          ? user.documents.toObject()
          : user.documents || {};
      ...
      updatedUser = await CombinedUser.findByIdAndUpdate(
        request.userId,
        { $set: updateField },
        { new: true, runValidators: true }
      );
      console.log("✅ CombinedUser.documents after update:", updatedUser?.documents);
    } else {
